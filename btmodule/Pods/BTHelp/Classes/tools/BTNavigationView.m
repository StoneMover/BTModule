...
+ (instancetype)createWithParentView:(UIView*)view height:(CGFloat)height{
    BTNavigationView * navView = [[BTNavigationView alloc] initWithFrame:CGRectZero navHeight:height];
    [view addSubview:navView];
    navView.translatesAutoresizingMaskIntoConstraints = NO;
    [navView bt_addTopToParent];
    [navView bt_addLeftToParent];
...
- (instancetype)initWithFrame:(CGRect)frame navHeight:(CGFloat)height{
    self = [super initWithFrame:frame];
...
- (void)initSelf{
    ...
    if (self.navHeight == 0) {
        self.navHeight = BTUtils.NAV_HEIGHT;
    }
    
    [self bt_addWidth:BTUtils.SCREEN_W];
    [self bt_addHeight:self.navHeight];
...
- (void)layoutSubviews{
    ...
    CGFloat top = self.navHeight - BTUtils.NAVCONTENT_HEIGHT;
...
- (void)setTitle:(NSString*)title{
    ...
    if (self.centerItem.type != BTNavigationItemStr || !self.centerItem) {
        self.centerItem = [BTNavigationItem itemWithTitle:title font:BTNavConfig.share.defaultNavTitleFont color:BTNavConfig.share.defaultNavTitleColor];
    }else{
...
+ (instancetype)itemWithTitle:(NSString*)title{
    return [self itemWithTitle:title font:[UIFont BTAutoFontWithSize:16 weight:UIFontWeightMedium] color:BTTheme.mainColor width:0];
}

+ (instancetype)itemWithTitle:(NSString*)title width:(CGFloat)width{
    return [self itemWithTitle:title font:[UIFont BTAutoFontWithSize:16 weight:UIFontWeightMedium] color:BTTheme.mainColor width:width];
}
...
- (instancetype)init{
    self = [super init];
    self.margin = BTNavConfig.share.navItemPadding;
    return self;
}
