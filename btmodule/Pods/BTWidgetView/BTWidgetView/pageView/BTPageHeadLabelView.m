...
- (UIView*)pageHeadView:(BTPageHeadView*)headView contentViewForIndex:(NSInteger)index{
    ...
    if (self.labelStyle == BTPageHeadViewStyleDefault) {
        ...
        if (self.itemWidth != 0){
            label.BTWidth = self.itemWidth;
        }
    }else{
        label.frame = CGRectMake(0, 0, self.BTWidth / self.titles.count, self.BTHeight);
    }
    if (self.labelDelegate && [self.labelDelegate respondsToSelector:@selector(BTPageHeadLabelView:label:index:)]) {
        [self.labelDelegate BTPageHeadLabelView:self label:label index:index];
    }
