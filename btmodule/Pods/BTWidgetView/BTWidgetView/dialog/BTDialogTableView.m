...
- (void)initShowView{
    ...
    self.cellHeight = 40;
    self.isNeedHead = YES;
    CGFloat rootViewW=self.locationTable==BTDialogLocationCenter?[UIScreen mainScreen].bounds.size.width-50*2:[UIScreen mainScreen].bounds.size.width;
    ...
    self.headView=[[BTDialogTableHeadView alloc] initWithFrame:CGRectMake(0, 0, self.rootView.BTWidth, 45)];
    [self.headView.btnCancel addTarget:self action:@selector(closeClick) forControlEvents:UIControlEventTouchUpInside];
    
    self.tableView=[[UITableView alloc] init];
    self.tableView.frame=CGRectMake(0, self.headView.BTBottom, self.rootView.BTWidth, 0);
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.cellLabelFont) {
        cell.labelContent.font = self.cellLabelFont;
    }
    if (self.cellLabelColor) {
        cell.labelContent.textColor = self.cellLabelColor;
    }
    return cell;
}
